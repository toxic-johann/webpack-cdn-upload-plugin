...
class WebpackCdnUploadPlugin {
                    ...
                    if (this.replaceUrlInCss && /.css$/.test(filename)) {
                        ...
                        for (const match of matches) {
                            ...
                            if (uploadedUrl && isString(uploadedUrl)) {
                                replacedSource = replacedSource.replace(new RegExp(String.raw `(${escapeStringRegexp(match[1])}`, 'g'), `(${uploadedUrl}`);
                            }
                        }
            ...
            if (this.replaceAssetsInHtml) {
                ...
                for (const htmlFilename of htmlFilenames) {
                    ...
                    for (const resourceName of Object.keys(assets)) {
                        ...
                        const nameWithPublicPathRegExp = new RegExp(`${escapeStringRegexp(this.originPublicPath)}((${this.uniqueMark})+.+${this.uniqueMark})?${escapeStringRegexp(resourceName)}((${this.uniqueMark})+)?`);
                        const match = replacedSource.match(nameWithPublicPathRegExp);
                        if (match) {
                            const uploadedUrl = this.chunksNameUrlMap[nameWithPublicPath];
                            /* istanbul ignore if  */
                            if (uploadedUrl) {
                                replacedSource = replacedSource.replace(new RegExp(String.raw `${escapeStringRegexp(match[0])}`, 'g'), uploadedUrl);
                            }
                        }
                    }
    ...
    markChunkName(compiler) {
        ...
        this.originChunkFilename = compiler.options.output.chunkFilename;
