...
exports.WebpackCdnUploadPlugin = void 0;
...
class WebpackCdnUploadPlugin {
    ...
    compilationFn(_, compilation) {
        ...
        if (this.replaceAssetsInHtml) {
            const beforeEmit = html_webpack_plugin_1.default.getHooks(compilation).beforeEmit;
            /* istanbul ignore if  */
            if (!beforeEmit) {
        ...
        while (sortedChunkGroups.length) {
            for (let i = sortedChunkGroups.length - 1; i > -1; i--) {
                ...
                const uploadAble = chunkGroup
                    .getChildren()
                    .reduce((uploadAble, childChunkGroup) => uploadAble && sortedChunkGroups.indexOf(childChunkGroup) === -1, true);
                /* istanbul ignore if  */
                if (!uploadAble)
                    continue;
    ...
    replaceAsyncChunkMapOfChunk(chunkGroup, compilation) {
        ...
        chunkGroup.chunks.forEach((chunk) => {
            ...
            const source = originSource.replace(new RegExp(`[a-zA-Z_]+.p\\s?\\+\\s?"${this.uniqueMark}"(.*?)"${this.uniqueMark}[^"]*"(.*?)"\\.js${this.uniqueMark}"`, 'g'), (_, $1) => {
...
exports.WebpackCdnUploadPlugin = WebpackCdnUploadPlugin;
